...
import { Definition, InfoText, Restraint } from "../RestraintProperty"
...
export namespace Prototype {
    function FullNameOf(nameLambda: () => any){
        return `${RootNamespace}.${NameOf(() => Prototype)}.${NameOf(nameLambda)}`
    }
...
export const DroneSet =
    Seq(Object.values<Definition>(Prototype))
        .groupBy(def => def.Data.name)
        .map(seq => seq.first() as Definition)

export function Register() {
    for(const def of DroneSet.values()){
